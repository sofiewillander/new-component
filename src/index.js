...
const testTemplatePath = `./templates/${options.lang}.test.js`;

// Get all of our file paths worked out, for the user's project.
const componentDir = `${options.dir}/${componentName}`;
const filePath = `${componentDir}/${componentName}.${fileExtension}`;
const testFilePath = `${componentDir}/${componentName}.test.${fileExtension}`;
...
mkDirPromise(componentDir)
  ...
  .then((testTemplate) =>
    // Format it using prettier, to ensure style consistency, and write to file.
    writeFilePromise(testFilePath, prettify(testTemplate))
  )
  .then((testTemplate) => {
    logItemCompletion("Test built and saved to disk.");
    return testTemplate;
  })
  .then((template) =>
